...
<div class="min-h-screen bg-default py-6 flex flex-col justify-center sm:py-12">
	<div class="relative py-3 sm:max-w-xl sm:mx-auto">
		<div class="relative px-4 py-10 bg-base-200 mx-8 md:mx-0 shadow rounded-3xl sm:p-10">
			<div class="max-w-md mx-auto">
				<div class="flex items-center space-x-5">
					...
					<div class="block pl-2 font-semibold text-xl self-start">
						<h2 class="uppercase font-bold leading-relaxed">Create an account</h2>
						<p class="text-sm opacity-80 leading-relaxed">
							And let us help you find your next dream job ✨
						</p>
					</div>
				</div>
				<AuthForm
					on:submit={async (event) => {
						...
						const createResponse = await api.account.create(
							'unique()',
							formData.email,
							formData.password
						);
						...
						if (createResponse.$id) {
							const loginResponse = await api.account.createSession(
								formData.email,
								formData.password
							);
				...
				>
					<div slot="details" class="flex">
						<a href="/login" class="link-hover">Already have an account? </a>
					</div>
				</AuthForm>
			</div>
		</div>
